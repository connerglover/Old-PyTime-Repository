...
def main(stdscr):
    ...
    for i in range(segment_count):
        ...
        if 'time' in globals():
            time = (d(debug_info_end_dict['cmt']) - d(debug_info_start_dict['cmt'])) + d(time)
        else:
            time = (d(debug_info_end_dict['cmt']) - d(debug_info_start_dict['cmt']))
    #prints menu
    ...
    while in_menu:
        ...
        if menu_select == '1':
            ...
            if not 'menu_current' in globals():
                continue
            elif menu_current == 1:
